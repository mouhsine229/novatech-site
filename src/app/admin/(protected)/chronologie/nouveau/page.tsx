import Link from "next/link";
import { ArrowLeft } from "lucide-react";
import { TimelineForm } from "../timeline-form";
import { createTimelineEvent } from "../actions";

export const metadata = { title: "Nouvel événement — Administration NOVATECH" };

export default function NewTimelineEventPage() {
  return (
    <div className="mx-auto max-w-2xl p-8">
      <Link href="/admin/chronologie" className="inline-flex items-center gap-1 font-mono text-xs uppercase tracking-wider text-slate hover:text-signal">
        <ArrowLeft className="h-3 w-3" aria-hidden="true" /> Retour à la chronologie
      </Link>
      <h1 className="mt-4 font-display text-2xl font-bold text-ink">Nouvel événement</h1>

      <div className="mt-8">
        <TimelineForm onSubmit={createTimelineEvent} />
      </div>
    </div>
  );
}
